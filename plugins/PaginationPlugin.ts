import { PhtpsPlugin, HttpClientConfig, HttpResponse, PaginateConfig } from '../config/types';

const extractCursor = (config: PaginateConfig, res: HttpResponse): string | number | null => {
  if (config.getNextCursor) return config.getNextCursor(res);
...
export const PaginationPlugin = (): PhtpsPlugin => {
  return {
    name: 'pagination',
    install: (client) => {
      const manager = (client as any).paginationManager;
      if (!manager) return;
      ...
      manager.registerStrategy('cursor', {
        computeNextParams: (config: HttpClientConfig, response: HttpResponse) => {
          if (!config.paginate) return null;
      ...
      manager.addOnResponseHook((response: HttpResponse) => {
        const config = response.config;
        if (!config.paginate || config.__isPrefetch) return;
        ...
        if (mode === 'prefetch') {
          ...
          if (depth < prefetchLimit) {
            ...
            if (!nextParams || !manager.hasNextPage(paginateConfig, response, [])) return;
            ...
            const executePrefetch = () => {
              ...
              if (activePrefetches >= maxQueue) return;

              activePrefetches++;
              const prefetchConfig: HttpClientConfig = {
                ...config,
            ...
            if (timingStrategy === 'idle' && typeof window !== 'undefined' && 'requestIdleCallback' in window) {
              (window as any).requestIdleCallback(() => executePrefetch());
