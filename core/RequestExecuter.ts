import { HttpClientConfig, HttpResponse, HttpError, HttpStreamResponse } from '../config/types';
import { UrlBuilder } from './UrlBuilder';
import { SimpleCrypto } from '../utils/SimpleCrypto';
import { StreamReader } from './StreamReader';

export class RequestExecutor {
  public static async execute<T>(config: HttpClientConfig): Promise<HttpResponse<T> | HttpStreamResponse<T>> {
    const safeConfig = { ...config };
    delete safeConfig.encryptionKey;

    if (config.signal?.aborted) {
      const error: HttpError = new Error('Request cancelled');
      error.isCancel = true;
    ...
    if (onUploadProgress && typeof XMLHttpRequest !== 'undefined') {
      return this.executeWithXHR<T>(fullUrl, config);
    }
    ...
    try {
      ...
      let finalBody = body;
      let finalHeaders = new Headers(headers as HeadersInit);
      ...
      if (config.stream) {
        ...
        const streamResponse: HttpStreamResponse<T> = {
          data: StreamReader.transform<T>(response, config),
        ...
        if (!response.ok) {
          ...
          error.response = streamResponse as unknown as HttpResponse;
          error.config = safeConfig;
          throw error;
        }
      ...
      const httpError: HttpError = error instanceof Error ? error : new Error(errorMessage);
  ...
  private static async readWithProgress(response: Response, onProgress: (ev: any) => void): Promise<any> {
  ...
  private static async executeWithXHR<T>(url: string, config: HttpClientConfig): Promise<HttpResponse<T>> {
    const safeConfig = { ...config };
    delete safeConfig.encryptionKey;
    ...
    return new Promise((resolve, reject) => {
      if (signal && signal.aborted) {
        const error: any = new Error('Request cancelled');
        error.isCancel = true;
        error.config = safeConfig;
        return reject(error);
      }
      ...
      xhr.onload = async () => {
        try {
          ...
          let responseData: any = xhr.response;
          try {
          ...
          if (xhr.status >= 200 && xhr.status < 300) {
            ...
            const error: HttpError = new Error(`HTTP Error: ${xhr.status}`);
    ...
    xhr.ontimeout = () => {
      cleanup();
      const error: HttpError = new Error(`Request timed out after ${timeout}ms`);
        error.isTimeout = true;
        error.config = safeConfig;
        reject(error);
      };

      xhr.send(finalBody as any);
    });
  }
}
