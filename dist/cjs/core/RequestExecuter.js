...
class RequestExecutor {
    static async execute(config) {
        ...
        const fullUrl = _fullUrl || UrlBuilder_1.UrlBuilder.build(config);
        ...
        try {
            ...
            if (encryptPayload && body && !alreadyEncrypted) {
                ...
                const encrypted = await SimpleCrypto_1.SimpleCrypto.encrypt(body, activeEncryptionKey);
            ...
            if (config.stream) {
                ...
                const streamResponse = {
                    data: StreamReader_1.StreamReader.transform(response, config),
            ...
            if (decryptResponse && response.headers.get('X-Phtps-Encrypted') === 'true') {
                ...
                if (responseData && responseData.data) {
                    responseData = await SimpleCrypto_1.SimpleCrypto.decrypt(responseData.data, activeEncryptionKey);
                }
                else if (typeof responseData === 'string') {
                    responseData = await SimpleCrypto_1.SimpleCrypto.decrypt(responseData, activeEncryptionKey);
                }
    ...
    static async executeWithXHR(url, config) {
        ...
        if (encryptPayload && body && encryptionKey && !alreadyEncrypted) {
            const encrypted = await SimpleCrypto_1.SimpleCrypto.encrypt(body, encryptionKey);
            finalBody = JSON.stringify({ data: encrypted });
            finalHeaders.set('Content-Type', 'application/json');
            finalHeaders.set('X-Phtps-Encrypted', 'true');
        }
